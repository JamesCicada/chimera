...
module.exports = {
    ...
    run: async (client, interaction) => {
        ...
        let guildData = await gDb.findOne({
        ...
        if (!guildData) {
            ...
            guildData = await gDb.findOne({
                guildId: interaction.guild.id,
            });
        }
        if(guildData.disabledCommands?.includes(interaction.commandName)) return interaction.reply({
            content: "This command is blacklisted!",
            ephemeral: true
        });;
        if(guildData.disabledChannels?.includes(interaction.channelId)) return interaction.reply({
            content: "This channel is blacklisted!",
            ephemeral: true
        });
        if(command.options?.premium && !guildData.premium["status"]) return interaction.reply({
            content: `You need premium tier ${command.options?.premium} to use this command or vote for the bot to get premium for 12h (must be in support server)!`,
            ephemeral: true
        }); 
        if(guildData.premium["tier"] < command.options?.premium) return interaction.reply({
            content: `You need premium tier ${command.options?.premium} to use this command or vote for the bot to get premium for 12h (must be in support server)!`,
            ephemeral: true
        });
        try {
