  ...
  module.exports = {
    ...
    run: async (client, interaction) => {
      ...
      if (!interaction.member.voice?.channelId) {
        return interaction.reply({
          content: `skip what? Please join a voice channel first!`,
          ephemeral: true,
        });
      }
      const queue = distube.getQueue(interaction.guild);
        ...
        const songNum = interaction.options.getInteger("number") - 1 || 1;
        if(songNum > queue.songs.length) {
          return interaction.reply({
            content: `Invalid song number!`,
            ephemeral: true
          })
        }
        await distube.jump(interaction.guild, songNum);
        interaction.reply({content: `Skipped song!`, ephemeral: true})
    },
  };
