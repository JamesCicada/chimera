const {
  ...
  module.exports = {
    category: "music",
    usage: "shuffle [track/queue/off]",
    structure: new SlashCommandBuilder()
      .setName("shuffle")
      .setDescription("shuffle the current track/queue"),
    options: {
      premium: 1,
    },
    /**
    ...
    run: async (client, interaction) => {
      ...
      if (!interaction.member.voice?.channelId) {
        return interaction.reply({
          content: `Shuffle what? Please join a voice channel first!`,
          ephemeral: true,
        });
      }
      ...
      if(queue.songs.length < 2) {
        return interaction.reply({
          content: `There is only 1 song in the queue!`,
          ephemeral: true,
        });
      }
